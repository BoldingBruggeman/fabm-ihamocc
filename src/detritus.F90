...
module ihamocc_detritus
   ...
   type, extends(type_base_model), public :: type_ihamocc_detritus
      ...
      type (type_state_variable_id) :: id_fdust
      type (type_diagnostic_variable_id) :: id_wopal, id_wcal, id_wpoc, id_wdust, id_bkopal, id_rem, id_remin2o, id_delcar, id_rdnit1, id_remin
      
      logical  :: AGG, WLIN, with_dmsph
      real(rk) :: remido, bkopal, ropal, rcalc, calmax, drempoc, dremopal, dremn2o, dremsul, dms_gamma, dmsp1, dmsp2, dmsp3, dmsp4, dmsp5, dmsp6, relaxfe, nmldmin
      real(rk) :: claydens, safe, SinkExp, cellsink, alow1, alar1, Stick, wmin, wmax, wline, wpoc, wcal, wopal
...
contains
   ...
   subroutine initialize(self, configunit)
      ...
      if self%AGG then
        ...
        call self%get_parameter(self%shear, 'shear', 'd-1', 'hear in the mixed layer', default=43200._rk)
        
        call self%register_state_variable(self%id_nos, 'nos','g-1', 'marine snow aggregates per g sea water')
        call self%register_state_variable(self%id_adust, 'adust','g-1', 'dust aggregates per g sea water')
      ...
      call self%register_state_variable(self%id_silica, 'silica', 'kmol/m^3', 'Silicid acid (Si(OH)4)')
      ...
      call self%register_dependency(self%id_wdust_in, 'wdust', 'm/s', 'dust sinking speed')
      ...
      call self%register_diagnostic_variable(self%id_wpoc, 'wpoc', 'm d-1', 'poc sinking speed')
      call self%register_diagnostic_variable(self%id_bkopal, 'bkopal', 'kmol Si/m3','half sat. constant for opal') !for bromo.f90
      call self%register_diagnostic_variable(self%id_pocrem, 'pocrem', 'kmol/m^3 d-1', 'deep remineralization of POC') !for cisonew.f90 & natdic.f90
      call self%register_diagnostic_variable(self%id_docrem, 'docrem', 'kmol/m^3 d-1', 'deep remineralization of DOC') !for cisonew.f90 & natdic.f90
      call self%register_diagnostic_variable(self%id_remin2o, 'remin2o', 'kmol/m^3 d-1', 'remin2o') !for cisonew.f90 & natdic.f90
      call self%register_diagnostic_variable(self%id_remin, 'remin', 'kmol/m^3 d-1', 'remin') !for cisonew.f90 & natdic.f90
      call self%register_diagnostic_variable(self%id_delcar_part, 'delcar_part', '-', 'calcium fraction of export') !for cisonew.f90 
      call self%register_diagnostic_variable(self%id_delcar, 'delcar', 'kmol/m^3 d-1', 'delcar') !for natdic.f90
      call self%register_diagnostic_variable(self%id_delsil, 'delsil', 'kmol/m^3 d-1', 'delsil') !for natdic.f90
   ...
   subroutine do(self, _ARGUMENTS_DO_)
      ...
      real(rk) :: anosloss, nos, exud, graton, grawa, nos_roc, zmornos, pocrem, docrem, phyrem, pommor, dimmor
      real(rk) :: dissopal, opal, iron, pocrem, phyrem, doc_roc, phosph_roc, ano3_roc, det_roc
      real(rk) :: sco212_roc, alkali_roc, oxygen_roc, calc_roc, silica_roc, opal_roc, iron_roc, fdust, remin2o
      ...
      _LOOP_BEGIN_                               
         ...
         _SET_DIAGNOSTIC_(self%id_bkopal, sel%bkopal) !pass bkopal parameter to bromo module.
   ...
   subroutine get_vertical_movement(self, _ARGUMENTS_GET_VERTICAL_MOVEMENT_)
      ...
      _LOOP_BEGIN_
         ...
         _ADD_VERTICAL_VELOCITY_(self%id_det, -wpoc)
         _ADD_VERTICAL_VELOCITY_(self%id_calc, -wcal)
         _ADD_VERTICAL_VELOCITY_(self%id_opal, -wopal)
         _ADD_VERTICAL_VELOCITY_(self%id_fdust, -wdust)
         
         if (self%AGG) then
             _GET_(id_wnos_in, wnos)
             _ADD_VERTICAL_VELOCITY_(self%id_phy, -wpoc)
             _ADD_VERTICAL_VELOCITY_(self%id_nos, -wnos)
             _ADD_VERTICAL_VELOCITY_(self%id_adust, -wpoc)
         endif
      _LOOP_END_
   ...
   subroutine check_state(self, _ARGUMENTS_CHECK_STATE_)
      ...
      real(rk) :: phy, det, nos, klme, pupper, plower, avmass, snow, nos_new
